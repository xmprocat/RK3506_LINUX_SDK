...
#if defined(SOC_RK3506) && defined(HAL_VOP_MODULE_ENABLED)
                           ...
                           uint32_t *lut, uint16_t lut_size)
{
    ...
    if (winId != VOP_WIN1) {
        HAL_DBG_ERR("Invalid winId\n");

        return HAL_INVAL;
    }

    for (i = 0; i < lut_size; i++) {
        VOP_Write(&s_vopRegMir.GAMMA_LUT[i], &pReg->GAMMA_LUT[i], lut[i]);
    }

    VOP_MaskWrite(&s_vopRegMir.DSP_CTRL2, &pReg->DSP_CTRL2, VOP_DSP_CTRL2_SW_DSP_LUT_EN_SHIFT,
                  VOP_DSP_CTRL2_SW_DSP_LUT_EN_MASK, 1);
...
{
    VOP_MaskWrite(&s_vopRegMir.DSP_CTRL2, &pReg->DSP_CTRL2, VOP_DSP_CTRL2_SW_DSP_LUT_EN_SHIFT,
                  VOP_DSP_CTRL2_SW_DSP_LUT_EN_MASK, 0);
                            ...
                            struct DISPLAY_MODE_INFO *pModeInfo)
{
    ...
    VOP_MaskWrite(&s_vopRegMir.WIN1_CTRL0 + regOffset,
                  &pReg->WIN1_CTRL0 + regOffset,
                  VOP_WIN1_CTRL0_WIN1_RB_SWAP_SHIFT,
                  VOP_WIN1_CTRL0_WIN1_RB_SWAP_MASK, pWinState->rbSwap);
