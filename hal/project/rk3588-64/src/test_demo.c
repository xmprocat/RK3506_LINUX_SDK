...
#error "irq preempt test need defined HAL_GIC_PREEMPT_FEATURE_ENABLED in hal_conf.h"
#endif
 ...
 */
    ...
    GIC_AMP_IRQ_CFG_ROUTE(UART5_IRQn, 0xd0, CPU_GET_AFFINITY(3, 0)),
    GIC_AMP_IRQ_CFG_ROUTE(RSVD0_IRQn, 0x80, CPU_GET_AFFINITY(3, 0)),
#endif
...
{
    printf("softirq_test:\n");
    HAL_IRQ_HANDLER_SetIRQHandler(irq, soft_isr, NULL);
    HAL_GIC_Enable(irq);
...
/************************************************/
#ifdef IRQ_PREEMPT_TEST
static void uart5_isr(int vector, void *param)
{
    printf("enter uart5_isr, vector = %d\n", vector);
    HAL_GIC_SetPending(RSVD0_IRQn);
    printf("leave uart5_isr, vector = %d\n", vector);
}

static void rsvd0_isr(int vector, void *param)
{
    printf("enter rsvd0_isr, vector = %d\n", vector);
    printf("leave rsvd0_isr, vector = %d\n", vector);
}

static void irq_preempt_test(void)
{
    printf("irq_preempt_test:\n");
    HAL_IRQ_HANDLER_SetIRQHandler(UART5_IRQn, uart5_isr, NULL);
    HAL_GIC_Enable(UART5_IRQn);
    HAL_IRQ_HANDLER_SetIRQHandler(RSVD0_IRQn, rsvd0_isr, NULL);
    HAL_GIC_Enable(RSVD0_IRQn);

    HAL_GIC_SetPending(UART5_IRQn);
}
#endif
