...
typedef struct _rkMpiVENCCtx {
    ...
    RK_BOOL    bSliceSplit;
    RK_S32     s32ThreshSad;
    RK_S32     s32ThreshMove;
    RK_S32     s32SwitchSad;
    RK_S32     s32SquarePct;
    RK_U32     u32JpegSplit;
    RK_BOOL    bQbias;
    RK_U32     u32FilterStrengthI;
    RK_U32     u32FilterStrengthP;
    RK_U32     u32MotionDeblurStrength;
    RK_U32     u32CuDqp;
    RK_U32     u32AntiRing;
    RK_U32     u32AntiLine;
    RK_U32     u32Lambda;
...
static struct combo_th g_combo_th[8];

static RK_U64 venc_TEST_COMM_GetNowUs() {
...
static RK_S32 create_ivs(TEST_VENC_CTX_S *ctx) {
    ...
    attr.u32MaxWidth = ctx->u32SrcWidth;
    attr.u32MaxHeight = ctx->u32SrcHeight;
    ...
    attr.s32ODPercent = 7;
    return RK_MPI_IVS_CreateChn(ctx->s32IvsChn, &attr);
}

static RK_S32 destroy_ivs(TEST_VENC_CTX_S *ctx) {
    RK_MPI_IVS_DestroyChn(ctx->s32IvsChn);
    return RK_SUCCESS;
}
...
static void *venc_jpeg_get_packet(void *arg) {
    ...
    while (combo_th->run) {
        ...
        if (s32Ret == RK_SUCCESS) {
            if (combo_th->dstFilePath != RK_NULL) {
                char filename[128];
                FILE *fp;
                snprintf(filename, sizeof(filename), "%s/venc_ch%d_%d.jpg", combo_th->dstFilePath, s32ChnId, loopCount);
            ...
            RK_U64 nowUs = venc_TEST_COMM_GetNowUs();
...
static RK_S32 create_venc_combo(TEST_VENC_CTX_S *ctx, RK_U32 u32Ch, RK_U32 s32ComboChnId) {
    ...
    g_combo_th[u32Ch].dstFilePath = ctx->dstFilePath;
    pthread_create(&g_combo_th[u32Ch].th, NULL, venc_jpeg_get_packet, &g_combo_th[u32Ch]);

    return RK_SUCCESS;
}

static RK_S32 destroy_venc_combo(TEST_VENC_CTX_S *ctx, RK_U32 u32Ch) {
    RK_S32 s32Ret = RK_SUCCESS;
...
void* venc_force_idr(void *pArgs) {
    TEST_VENC_CTX_S *pstCtx     = (TEST_VENC_CTX_S *)(pArgs);
...
void* venc_send_frame(void *pArgs) {
    ...
    while (!pstCtx->threadExit) {
        ...
        if (pstCtx->u32DstCodec == RK_VIDEO_ID_JPEG && pstCtx->u32JpegSplit > 0 && u32TimeRef % pstCtx->u32JpegSplit == 0)
        ...
        stFrame.stVFrame.u32TimeRef = u32TimeRef++;
        ...
        if (pstCtx->bInsertUserData) {
            ...
            RK_MPI_VENC_InsertUserData(u32Ch, (RK_U8 *)(user_data2), user_len);
        }
__RETRY:
        if (pstCtx->bSendFrameEx) {
            ...
            if (pstCtx->bIvs) {
                s32Ret = RK_MPI_IVS_SendFrame(pstCtx->s32IvsChn, &stFrame, -1);
                IVS_RESULT_INFO_S stResults;
                memset(&stResults, 0, sizeof(IVS_RESULT_INFO_S));
                s32Ret = RK_MPI_IVS_GetResults(pstCtx->s32IvsChn, &stResults, -1);
                if (s32Ret == RK_SUCCESS) {
                    if (stResults.s32ResultNum == 1) {
                        int x = RK_ALIGN(pstCtx->u32SrcWidth / 8 / 8, 4) / 4;
                        int y = RK_ALIGN(pstCtx->u32SrcHeight / 8, 4) / 4;
                        int bit = pstCtx->u32SrcWidth / 8 / 4;
                        printf("MD u32Square: %u\n", stResults.pstResults->stMdInfo.u32Square);
                        if (stResults.pstResults->stMdInfo.pData &&
                            stResults.pstResults->stMdInfo.u32Square >
                            pstCtx->u32SrcWidth * pstCtx->u32SrcHeight * pstCtx->s32SquarePct / 100) {
                            for (int n = 0; n < x * 8; n++)
                    ...
                    RK_MPI_IVS_ReleaseResults(pstCtx->s32IvsChn, &stResults);
                } else {
...
RK_S32 unit_test_mpi_venc(TEST_VENC_CTX_S *ctx) {
    ...
    if (ctx->bIvs) {
        ...
        s32Ret = RK_MPI_IVS_GetMdAttr(ctx->s32IvsChn, &stMdAttr);
        if (s32Ret) {
            RK_LOGE("ivs get mdattr failed:%x", s32Ret);
            return RK_FAILURE;
        }
        stMdAttr.s32ThreshSad = ctx->s32ThreshSad;
        stMdAttr.s32ThreshMove = ctx->s32ThreshMove;
        stMdAttr.s32SwitchSad = ctx->s32SwitchSad;
        stMdAttr.bFlycatkinFlt = RK_TRUE;
        stMdAttr.s32ThresDustMove = 3;
        stMdAttr.s32ThresDustBlk = 3;
        stMdAttr.s32ThresDustChng = 50;
        printf("s32ThreshSad %d, s32ThreshMove %d, s32SwitchSad %d\n",
               ctx->s32ThreshSad, ctx->s32ThreshMove, ctx->s32SwitchSad);
        s32Ret = RK_MPI_IVS_SetMdAttr(ctx->s32IvsChn, &stMdAttr);
        if (s32Ret) {
            RK_LOGE("ivs set mdattr failed:%x", s32Ret);
            return RK_FAILURE;
        }
        char name[128];
        snprintf(name, sizeof(name), "/tmp/ivs%d_result.txt", ctx->s32IvsChn);
        ctx->fp_ivs_result = fopen(name, "wb");
    ...
    for (u32Ch = ctx->u32ChnStart; u32Ch < ctx->u32ChnStart + ctx->u32ChNum; u32Ch++) {
        ...
        stAttr.stVencAttr.u32PicWidth = ctx->u32SrcWidth;
        stAttr.stVencAttr.u32PicHeight = ctx->u32SrcHeight;
        if (stAttr.stVencAttr.enType == RK_VIDEO_ID_JPEG && ctx->u32JpegSplit > 0)
            stAttr.stVencAttr.u32PicHeight *= ctx->u32JpegSplit;
        ...
        stAttr.stVencAttr.u32VirHeight = ctx->u32srcVirHeight;
        if (stAttr.stVencAttr.enType == RK_VIDEO_ID_JPEG && ctx->u32JpegSplit > 0)
            stAttr.stVencAttr.u32VirHeight *= ctx->u32JpegSplit;
        ...
        if (ctx->enGopMode) {
            ...
            if (stAttr.stGopAttr.enGopMode > VENC_GOPMODE_NORMALP && stAttr.stGopAttr.enGopMode < VENC_GOPMODE_SMARTP)
                stAttr.stGopAttr.u32TsvcPreload = 1;
            RK_MPI_VENC_SetChnAttr(u32Ch, &stAttr);
        }
        ...
        if (stAttr.stVencAttr.enType == RK_VIDEO_ID_JPEG) {
            ...
            if (ctx->u32FixQp)
                ...
                stJpegParam.u32Qfactor = 77;
            if (ctx->u32JpegSplit > 0)
                stJpegParam.u32MCUPerECS = RK_ALIGN_16(ctx->u32SrcWidth) / 16 * RK_ALIGN_16(ctx->u32SrcHeight) / 16;
            RK_MPI_VENC_SetJpegParam(u32Ch, &stJpegParam);
        }
        ...
        RK_U32 u32Strength = 0;
        RK_MPI_VENC_GetMotionDeblurStrength(u32Ch, &u32Strength);
        if (ctx->u32MotionDeblurStrength != u32Strength) {
            RK_MPI_VENC_SetMotionDeblurStrength(u32Ch, ctx->u32MotionDeblurStrength);
        }

        VENC_H265_CU_DQP_S stCuDqp;
        memset(&stCuDqp, 0, sizeof(stCuDqp));
        RK_MPI_VENC_GetH265CuDqp(u32Ch, &stCuDqp);
        if (ctx->u32CuDqp <= 2) {
            stCuDqp.u32CuDqp = ctx->u32CuDqp;
            RK_MPI_VENC_SetH265CuDqp(u32Ch, &stCuDqp);
        }

        VENC_ANTI_RING_S stAntiRing;
        memset(&stAntiRing, 0, sizeof(stAntiRing));
        RK_MPI_VENC_GetAntiRing(u32Ch, &stAntiRing);
        if (ctx->u32AntiRing <= 3) {
            stAntiRing.u32AntiRing = ctx->u32AntiRing;
            RK_MPI_VENC_SetAntiRing(u32Ch, &stAntiRing);
        }

        VENC_ANTI_LINE_S stAntiLine;
        memset(&stAntiLine, 0, sizeof(stAntiLine));
        RK_MPI_VENC_GetAntiLine(u32Ch, &stAntiLine);
        if (ctx->u32AntiLine <= 3) {
            stAntiLine.u32AntiLine = ctx->u32AntiLine;
            RK_MPI_VENC_SetAntiLine(u32Ch, &stAntiLine);
        }

        VENC_LAMBDA_S stLambda;
        memset(&stLambda, 0, sizeof(stLambda));
        RK_MPI_VENC_GetLambda(u32Ch, &stLambda);
        if (ctx->u32Lambda <= 8) {
            stLambda.u32Lambda = ctx->u32Lambda;
            RK_MPI_VENC_SetLambda(u32Ch, &stLambda);
        }
        ...
        if (ctx->bSliceSplit) {
            ...
            stSliceSplit.u32SplitSize = 960;
            RK_MPI_VENC_SetSliceSplit(u32Ch, &stSliceSplit);
        }

        if (ctx->bQbias) {
            if (stAttr.stVencAttr.enType == RK_VIDEO_ID_AVC) {
                VENC_H264_QBIAS_S stQbias;
                memset(&stQbias, 0, sizeof(stQbias));
                stQbias.bEnable = RK_TRUE;
                stQbias.u32QbiasI = 500;
                stQbias.u32QbiasP = 300;
                RK_MPI_VENC_SetH264Qbias(u32Ch, &stQbias);
            } else if (stAttr.stVencAttr.enType == RK_VIDEO_ID_HEVC) {
                VENC_H265_QBIAS_S stQbias;
                memset(&stQbias, 0, sizeof(stQbias));
                stQbias.bEnable = RK_TRUE;
                stQbias.u32QbiasI = 120;
                stQbias.u32QbiasP = 100;
                RK_MPI_VENC_SetH265Qbias(u32Ch, &stQbias);
            }
        }

        if (ctx->u32FilterStrengthI || ctx->u32FilterStrengthP) {
            VENC_FILTER_S stFilter;
            RK_MPI_VENC_GetFilter(u32Ch, &stFilter);
            stFilter.u32StrengthI = ctx->u32FilterStrengthI;
            stFilter.u32StrengthP = ctx->u32FilterStrengthP;
            RK_MPI_VENC_SetFilter(u32Ch, &stFilter);
        }

        if (ctx->bForceIdr)
            pthread_create(&forceIdrThread[u32Ch], 0, venc_force_idr, (void *)(&stVencCtx[u32Ch]));

        memcpy(&(stVencCtx[u32Ch]), ctx, sizeof(TEST_VENC_CTX_S));
	pthread_create(&vencThread[u32Ch], 0, venc_send_frame, (void *)(&stVencCtx[u32Ch]));
        if (ctx->bGetStream)
            pthread_create(&getStreamThread[u32Ch], 0, venc_get_stream, (void *)(&stVencCtx[u32Ch]));
    }

    if (ctx->bCombo)
        create_venc_combo(ctx, COMBO_START_CHN, ctx->u32ChnStart);
...
__FAILED:
    ...
    if (ctx->bCombo)
        destroy_venc_combo(ctx, COMBO_START_CHN);
...
int rk_mpi_venc_test(int argc, const char **argv) {
    RK_S32 s32Ret = RK_SUCCESS;
    TEST_VENC_CTX_S ctx;
    ...
    ctx.s32SwitchSad    = 2;
    ctx.s32SquarePct    = 35;
    ctx.u32MotionDeblurStrength = 3;
    ctx.u32CuDqp        = 0xFF;
    ctx.u32AntiRing     = 0xFF;
    ctx.u32AntiLine     = 0xFF;
    ctx.u32Lambda       = 0xFF;
    ...
    struct argparse_option options[] = {
        ...
        OPT_INTEGER('\0', "ivs_chn", &(ctx.s32IvsChn),
                    "ivs channel, default(0)", NULL, 0, 0),
        OPT_INTEGER('\0', "threshsad", &(ctx.s32ThreshSad),
                    "MD threshsad, default(40)", NULL, 0, 0),
        OPT_INTEGER('\0', "threshmove", &(ctx.s32ThreshMove),
                    "MD threshmove, default(2)", NULL, 0, 0),
        OPT_INTEGER('\0', "switchsad", &(ctx.s32SwitchSad),
                    "MD switchsad, default(2)", NULL, 0, 0),
        OPT_INTEGER('\0', "squarepct", &(ctx.s32SquarePct),
                    "MD square percent, default(35)", NULL, 0, 0),
        ...
        OPT_INTEGER('\0', "scene_mode", &(ctx.u32SceneMode),
                    "set scene mode(0: ipc, 1: sport dv, 2: cvr, 3: ipc_ptz), default(0)", NULL, 0, 0),
        OPT_INTEGER('\0', "slice_split", &(ctx.bSliceSplit),
                    "slice split test(0:disable 1:enable) default(0)", NULL, 0, 0),
        OPT_INTEGER('\0', "jpeg_split", &(ctx.u32JpegSplit),
                    "jpeg split (u32JpegSplit frames when split) default(0)", NULL, 0, 0),
        OPT_INTEGER('\0', "qbias", &(ctx.bQbias),
                    "qbias setting default(0)", NULL, 0, 0),
        OPT_INTEGER('\0', "filter_strength_i", &(ctx.u32FilterStrengthI),
                    "filter strength I setting default(0)", NULL, 0, 0),
        OPT_INTEGER('\0', "filter_strength_p", &(ctx.u32FilterStrengthP),
                    "filter strength P setting default(0)", NULL, 0, 0),
        OPT_INTEGER('\0', "motion_deblur_strength", &(ctx.u32MotionDeblurStrength),
                    "set motion deblur strength, default(3)", NULL, 0, 0),
        OPT_INTEGER('\0', "cu_dqp", &(ctx.u32CuDqp),
                    "set cu dqp, default(x)", NULL, 0, 0),
        OPT_INTEGER('\0', "anti_ring", &(ctx.u32AntiRing),
                    "set anti ring strength, default(x)", NULL, 0, 0),
        OPT_INTEGER('\0', "anti_line", &(ctx.u32AntiLine),
                    "set anti line strength, default(x)", NULL, 0, 0),
        OPT_INTEGER('\0', "lambda", &(ctx.u32Lambda),
                    "set lambda index, default(x)", NULL, 0, 0),
...
int main(int argc, const char **argv) {
    return rk_mpi_venc_test(argc, argv);
}
#endif

#ifdef OS_RTT
#include <finsh.h>
MSH_CMD_EXPORT(rk_mpi_venc_test, rockit venc module test);
#endif
