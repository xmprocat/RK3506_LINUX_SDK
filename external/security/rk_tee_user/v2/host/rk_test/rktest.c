...
static TEEC_Result invoke_keylad_test(TEEC_Session *session,
				 TEEC_Operation *operation, uint32_t *error_origin)
{
	return TEEC_InvokeCommand(session, RKTEST_TA_CMD_KEYLAD_TEST,
				  operation, error_origin);
}

static TEEC_Result invoke_keylad_write(TEEC_Session *session,
				 TEEC_Operation *operation, uint32_t *error_origin)
{
	return TEEC_InvokeCommand(session, RKTEST_TA_CMD_KEYLAD_WRITE,
				  operation, error_origin);
}

static TEEC_Result invoke_keylad_hmac(TEEC_Session *session,
				 TEEC_Operation *operation, uint32_t *error_origin)
{
	return TEEC_InvokeCommand(session, RKTEST_TA_CMD_KEYLAD_HMAC,
				  operation, error_origin);
}
...
TEEC_Result rk_test(uint32_t invoke_command)
{
	...
	case  DERIVE_KEY:
		...
		break;
	case  KEYLAD_TEST:
		res = invoke_keylad_test(&session, &operation, &error_origin);
		break;
	case  KEYLAD_WRITE:
		res = invoke_keylad_write(&session, &operation, &error_origin);
		break;
	case  KEYLAD_HMAC:
		res = invoke_keylad_hmac(&session, &operation, &error_origin);
		break;
	default:
