...
#define CRYPTO_SERVICE_CMD_KEYLAD_FLOW_NORMAL		0x00000004
#define CRYPTO_SERVICE_CMD_KEYLAD_FLOW_DERIVE		0x00000005
#define CRYPTO_SERVICE_CMD_USER_TA_OEM_OTP_KEY_HMAC	0x00000006
			...
			TEE_OperationMode mode)
{
	...
	TEE_ObjectType object_type = TEE_TYPE_RSA_KEYPAIR;
	...
	if (key->d_len == 0) {
		...
		priv_mode = 0;
		object_type = TEE_TYPE_RSA_PUBLIC_KEY;
	}
	...
	res = TEE_AllocateTransientObject(object_type, max_key_size, &obj);
	if (res != TEE_SUCCESS) {
		EMSG("TEE_AllocateTransientObject failed with code 0x%x", res);
		return res;
	}
			 ...
			 ec_key_t *key, uint32_t algo, TEE_OperationMode mode)
{
	...
	uint32_t max_key_size = 0;
	uint32_t priv_mode = 1;
	TEE_ObjectType object_type = TEE_TYPE_ECDSA_KEYPAIR;
	...
	if (key->d_len == 0) {
		...
		priv_mode = 0;
		object_type = TEE_TYPE_ECDSA_PUBLIC_KEY;
	}
		      ...
		      ec_key_t *key, uint32_t algo, TEE_OperationMode mode)
{
	...
	uint32_t max_key_size = 0;
	uint32_t priv_mode = 1;
	TEE_ObjectType object_type = TEE_TYPE_SM2_PKE_KEYPAIR;
	...
	if (key->d_len == 0) {
		...
		priv_mode = 0;
		object_type = TEE_TYPE_SM2_PKE_PUBLIC_KEY;
	}
			  ...
			  ec_key_t *key, uint32_t algo, TEE_OperationMode mode)
{
	...
	uint32_t max_key_size = 0;
	uint32_t priv_mode = 1;
	TEE_ObjectType object_type = TEE_TYPE_SM2_DSA_KEYPAIR;
	...
	if (key->d_len == 0) {
		...
		priv_mode = 0;
		object_type = TEE_TYPE_SM2_DSA_PUBLIC_KEY;
	}
...
{
	TEE_UUID sta_uuid = RK_CRYPTO_SERVICE_UUID;
	...
	if (!msg || msg_len == 0 || !mac || !mac_len || *mac_len == 0)
		return TEE_ERROR_BAD_PARAMETERS;
	...
	taParams[0].value.a = algo;

	taParams[1].value.a = otp_keyid;
	taParams[1].value.b = key_len;

	taParams[2].memref.buffer = msg;
	taParams[2].memref.size   = msg_len;

	taParams[3].memref.buffer = mac;
	taParams[3].memref.size   = *mac_len;

	res = TEE_InvokeTACommand(sta_session, 0,
				 CRYPTO_SERVICE_CMD_USER_TA_OEM_OTP_KEY_HMAC,
				 nParamTypes, taParams, &origin);
	if (res != TEE_SUCCESS && res != TEE_ERROR_NOT_SUPPORTED) {
		EMSG("TEE_InvokeTACommand returned 0x%x.\n", res);
	}

	if (res == TEE_SUCCESS)
		*mac_len = taParams[3].memref.size;
				 ...
				 keylad_flow_data *flow_data, keylad_data_t *da)
{
	...
	taParams[0].value.a       = keyid;
	taParams[1].memref.size   = sizeof(*tsp_cfg);
	taParams[1].memref.buffer = tsp_cfg;
	taParams[2].memref.size   = sizeof(*flow_data);
	taParams[2].memref.buffer = flow_data;
	taParams[3].memref.size   = sizeof(*da);
	taParams[3].memref.buffer = da;

	res = TEE_InvokeTACommand(sta_session, 0, CRYPTO_SERVICE_CMD_KEYLAD_FLOW_NORMAL,
				  nParamTypes, taParams, &origin);
				 ...
				 keylad_flow_data *flow_data, keylad_data_t *da)
{
	...
	res = TEE_InvokeTACommand(sta_session, 0, CRYPTO_SERVICE_CMD_KEYLAD_FLOW_DERIVE,
				  nParamTypes, taParams, &origin);
	if (res == TEE_ERROR_NOT_SUPPORTED) {
...
{
	TEE_UUID sta_uuid = STORAGE_UUID;
	TEE_TASessionHandle sta_session = TEE_HANDLE_NULL;
	uint32_t origin;
	TEE_Result res;
	TEE_Param taParams[4];
	...
	taParams[0].value.a = key_id;

	res = TEE_InvokeTACommand(sta_session, 0, STORAGE_CMD_SET_OEM_HR_OTP_READ_LOCK,
				  nParamTypes, taParams, &origin);
	if (res != TEE_SUCCESS)
		EMSG("TEE_InvokeTACommand returned 0x%x\n", res);
...
TEE_Result rk_esck_set_read_lock(enum RK_ESCK_KEYID key_id)
{
	...
	res = TEE_InvokeTACommand(sta_session, 0, STORAGE_CMD_SET_ESCK_KEY_MASK,
				  nParamTypes, taParams, &origin);
	if (res != TEE_SUCCESS)
		EMSG("TEE_InvokeTACommand returned 0x%x\n", res);
...
TEE_Result rk_is_oem_otp_key_written(enum RK_OEM_OTP_KEYID key_id, bool *is_written)
{
	...
	*is_written = false;
	...
	nParamTypes = TEE_PARAM_TYPES(TEE_PARAM_TYPE_VALUE_INOUT,
				      TEE_PARAM_TYPE_NONE,
				      TEE_PARAM_TYPE_NONE,
				      TEE_PARAM_TYPE_NONE);
	...
	res = TEE_InvokeTACommand(sta_session, 0, STORAGE_CMD_OEM_OTP_KEY_IS_WRITTEN,
				  nParamTypes, taParams, &origin);
	if (res &&
...
TEE_Result rk_is_esck_written(enum RK_ESCK_KEYID key_id, bool *is_written)
{
	...
	*is_written = false;
	...
	res = TEE_InvokeTACommand(sta_session, 0, STORAGE_CMD_ESCK_KEY_IS_WRITTEN,
				  nParamTypes, taParams, &origin);
	if (res &&
	    res != TEE_ERROR_NOT_SUPPORTED &&
	    res != TEE_ERROR_ACCESS_DENIED) {
...
TEE_Result rk_write_oem_otp_key(enum RK_OEM_OTP_KEYID key_id, uint8_t *key, uint32_t key_len)
{
	...
	taParams[0].value.a       = key_id;
	taParams[1].memref.size   = key_len;
	taParams[1].memref.buffer = key;

	res = TEE_InvokeTACommand(sta_session, 0, STORAGE_CMD_WRITE_OEM_OTP_KEY,
				  nParamTypes, taParams, &origin);
	if (res != TEE_SUCCESS) {
		EMSG("TEE_InvokeTACommand returned 0x%x\n", res);
	}
...
TEE_Result rk_write_esck(enum RK_ESCK_KEYID key_id, uint8_t *key, uint32_t key_len)
{
	...
	res = TEE_InvokeTACommand(sta_session, 0, STORAGE_CMD_WRITE_ESCK_KEY,
				  nParamTypes, taParams, &origin);
	if (res != TEE_SUCCESS) {
		EMSG("TEE_InvokeTACommand returned 0x%x\n", res);
	}
