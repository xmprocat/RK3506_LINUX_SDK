...
if [ -d "$TOOLCHAIN_PREBUILTS" ]; then
	...
	export ARM32_UCLIBC_PATH=arm-rockchip830-linux-uclibcgnueabihf
	export AARCH64_UCLIBC_PATH=aarch64-rockchip830-linux-uclibcgnueabihf
	if [ ! -d "$TOOLCHAIN_PATH_ARM32" ]; then
		TOOLCHAIN_PATH_ARM32=$TOOLCHAIN_PREBUILTS/arm-rockchip831-linux-uclibcgnueabihf/bin/
		CROSS_COMPILE32=arm-rockchip831-linux-uclibcgnueabihf-
		export ARM32_UCLIBC_PATH=arm-rockchip831-linux-uclibcgnueabihf
	fi
	if [ ! -d "$TOOLCHAIN_PATH_AARCH64" ]; then
		TOOLCHAIN_PATH_AARCH64=$TOOLCHAIN_PREBUILTS/aarch64-rockchip831-linux-uclibcgnu/bin/
		CROSS_COMPILE64=aarch64-rockchip831-linux-uclibcgnu-
		export AARCH64_UCLIBC_PATH=aarch64-rockchip831-linux-uclibcgnueabihf
	fi
else
