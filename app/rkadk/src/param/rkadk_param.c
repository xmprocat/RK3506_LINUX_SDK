...
static void RKADK_PARAM_DefRecAttr(RKADK_U32 u32CamId,
  ...
  pstAttr->venc_param.i_min_qp = -1;
  pstAttr->venc_param.frame_min_qp = -1;
  pstAttr->venc_param.frame_max_qp = -1;
  pstAttr->venc_param.i_frame_min_qp = -1;
  pstAttr->venc_param.i_frame_max_qp = -1;
...
static void RKADK_PARAM_DefStreamCfg(RKADK_U32 u32CamId, char *path,
  ...
  pstStreamCfg->attribute.venc_param.i_min_qp = -1;
  pstStreamCfg->attribute.venc_param.frame_min_qp = -1;
  pstStreamCfg->attribute.venc_param.frame_max_qp = -1;
  pstStreamCfg->attribute.venc_param.i_frame_min_qp = -1;
  pstStreamCfg->attribute.venc_param.i_frame_max_qp = -1;
...
static void RKADK_PARAM_Dump() {
  ...
  for (i = 0; i < (int)pstCfg->stCommCfg.sensor_count; i++) {
    ...
    for (j = 0; j < (int)pstCfg->stMediaCfg[i].stRecCfg.file_num; j++) {
      ...
      printf("\t\tsensor[%d] stRecCfg[%d] i_frame_max_qp: %d\n", i, j,
             pstCfg->stMediaCfg[i].stRecCfg.attribute[j].venc_param.i_frame_max_qp);
      printf("\t\tsensor[%d] stRecCfg[%d] frame_min_qp: %d\n", i, j,
             pstCfg->stMediaCfg[i].stRecCfg.attribute[j].venc_param.frame_min_qp);
      printf("\t\tsensor[%d] stRecCfg[%d] frame_max_qp: %d\n", i, j,
             pstCfg->stMediaCfg[i].stRecCfg.attribute[j].venc_param.frame_max_qp);
    ...
    printf("\t\tsensor[%d] stStreamCfg i_frame_max_qp: %d\n", i,
           pstCfg->stMediaCfg[i].stStreamCfg.attribute.venc_param.i_frame_max_qp);
    printf("\t\tsensor[%d] stStreamCfg frame_min_qp: %d\n", i,
           pstCfg->stMediaCfg[i].stStreamCfg.attribute.venc_param.frame_min_qp);
    printf("\t\tsensor[%d] stStreamCfg frame_max_qp: %d\n", i,
           pstCfg->stMediaCfg[i].stStreamCfg.attribute.venc_param.frame_max_qp);
    ...
    printf("\t\tsensor[%d] stLiveCfg i_frame_max_qp: %d\n", i,
           pstCfg->stMediaCfg[i].stLiveCfg.attribute.venc_param.i_frame_max_qp);
    printf("\t\tsensor[%d] stLiveCfg frame_min_qp: %d\n", i,
           pstCfg->stMediaCfg[i].stLiveCfg.attribute.venc_param.frame_min_qp);
    printf("\t\tsensor[%d] stLiveCfg frame_max_qp: %d\n", i,
           pstCfg->stMediaCfg[i].stLiveCfg.attribute.venc_param.frame_max_qp);
...
static RKADK_S32 RKADK_PARAM_SetRcParam(RKADK_PARAM_VENC_ATTR_S stVencAttr) {
  ...
  RKADK_S32 u32FrmMinQp, u32FrmMaxQp;
  RKADK_S32 u32FrmMinIQp, u32FrmMaxIQp;
  ...
  if (stVencAttr.codec_type == RKADK_CODEC_TYPE_H264) {
    ...
    u32FrmMaxQp = stRcParam.stParamH264.u32FrmMaxQp;
    u32FrmMinIQp = stRcParam.stParamH264.u32FrmMinIQp;
    u32FrmMaxIQp = stRcParam.stParamH264.u32FrmMaxIQp;
    ...
    u32FrmMaxQp = stRcParam.stParamH265.u32FrmMaxQp;
    u32FrmMinIQp = stRcParam.stParamH265.u32FrmMinIQp;
    u32FrmMaxIQp = stRcParam.stParamH265.u32FrmMaxIQp;
  ...
  if (stVencAttr.venc_param.i_frame_max_qp >= 0 && stVencAttr.venc_param.i_frame_max_qp <= 51) {
    u32FrmMaxIQp = stVencAttr.venc_param.i_frame_max_qp;
    bSetRcParam = true;
  }
  ...
  if (stVencAttr.venc_param.frame_max_qp >= 0 && stVencAttr.venc_param.frame_max_qp <= 51) {
    u32FrmMaxQp = stVencAttr.venc_param.frame_max_qp;
    bSetRcParam = true;
  }
